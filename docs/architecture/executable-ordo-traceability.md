...
Requirements version 1.1.0 (work item `FEAT-ECHELON-PROVENANCE-R12`) adopts
Praxis provenance contract revision 1.2.
...
| ORDO-PROV-06 conformance with the Praxis contract | Implemented | `ProvenanceBlock.classify` / `classifyText` / `appendJson` / `addLineage` / `addLineageJson` / `originator` / `withRole`; `ContributionKey.escapeSegment` / `ofOperation` / `ofEnvelopeV1`; `Json.parse` duplicate-member and surrogate refusal (`src/Ordo.Core/Json.fs`); `IdentifierNotWellFormed` (`src/Ordo.Core/Identifiers.fs`); fixtures in `tests/Ordo.Tests/fixtures/praxis-provenance/` | `ProvenanceTests`: SHA-256 against `SOURCE.json` (Praxis `b003718`, contract 1.2); all 70 conformance cases, 14 text cases, 12 envelope-key cases, 8 lineage cases; round trip; Echelon chain replay; appending rules; revision 1.1 rules 1, 2, 4, 5, 6 and 8; revision 1.2 findings 5, 10, 11 and rules 3, 4, 6; unsupported carried verbatim; malformed refused |
